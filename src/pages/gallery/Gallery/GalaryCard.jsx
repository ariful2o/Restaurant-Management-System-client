  ...
  return (
    <>
      <div className="relative inline-block" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <img src={Photo_URL} alt={Foood_Name} className={`block w-full h-full transition-opacity duration-500 ${isHovered ? 'opacity-60' : 'opacity-100'} ${isHovered ? 'scale-110' : 'scale-100'}`} />
        {isHovered && (
